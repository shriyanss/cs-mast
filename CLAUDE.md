...
**Special rule for declaration nodes:** `VariableDeclaration`, `FunctionDeclaration`, `ClassDeclaration`, `ImportDeclaration` always use their specific formulas (eq8–18). The `decl` scat flag is a *variant selector* (controls whether NodeType is included), not a gate. This differs from loops/conditionals where the category flag is a gate.
