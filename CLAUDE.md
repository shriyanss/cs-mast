...
> **Never delete `CHANGELOG.md`.** It is a permanent project artifact. Always add a new entry at the top; never remove existing ones.
...
| A6  | `VariableDeclarator` uses eq10/11 (NodeType always included) **only when `decl` is in scat**. When `decl` is absent and `VariableDeclarator` is not in sinc, it is transparent (A11).                                                                                                                                                                                                                                               |
...
| A11 | **Transparent passthrough for uncategorized nodes** (not covered by either scat or sinc, including declaration types when `decl` is absent): `computedHash = sha256(concat of direct children's non-undefined computedHash, source order)`. If no children have a hash, `computedHash` remains `undefined`. A subtree with no configured node types produces no hash at all. Uncategorized nodes are NOT added to the signatureMap. **Implementation history:** A11 was incorrectly implemented prior to v0.1.8 — uncategorized node types applied partial per-type formulas instead of pure child-hash propagation. v0.1.8 is the first correct implementation. All datasets and signatures generated before v0.1.8 are affected; results are not comparable across the v0.1.7→v0.1.8 boundary. |
...
**Declaration nodes gate:** `VariableDeclaration`, `FunctionDeclaration`, `ClassDeclaration`, `ImportDeclaration` use their specific formulas (eq8–18) **only when `decl` is in scat**. When `decl` is absent they are treated as uncategorized (transparent passthrough, A11). This differs from the previous "always apply formula" behavior. `VariableDeclarator` follows the same rule (see A6).
